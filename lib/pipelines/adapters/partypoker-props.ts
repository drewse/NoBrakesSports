/**
 * partypoker/partysports Ontario adapter.
 *
 * Entain CDS API at www.on.partysports.ca/cds-api/
 * Same platform as partypoker/bwin — shares fixture IDs and access ID.
 */

const BASE = 'https://www.on.partysports.ca/cds-api/bettingoffer'
...
const HEADERS: Record<string, string> = {
  ...
  'Referer': 'https://www.on.partysports.ca/',
  'Origin': 'https://www.on.partysports.ca',
}

// partypoker sport IDs → competition IDs we care about
export const PP_LEAGUES: { sportId: number; competitionId: number; leagueSlug: string; name: string }[] = [
...
async function fetchFixtureIds(sportId: number, competitionId: number): Promise<{ id: string; name: string; participants: any[]; startDate: string }[]> {
  ...
  try {
    ...
    console.error(`partypoker fixtures sportId=${sportId}:`, e)
    return []
  }
}
...
function parseFixtureMarkets(data: any, leagueSlug: string): PPResult | null {
  ...
  const gameMarkets: PPGameMarket[] = []
