/**
 * BetMGM Ontario adapter.
 *
 * Entain CDS API at www.on.betmgm.ca/cds-api/
...
const BASE = 'https://www.on.betmgm.ca/cds-api/bettingoffer'
...
const HEADERS: Record<string, string> = {
  ...
  'Referer': 'https://www.on.betmgm.ca/',
  'Origin': 'https://www.on.betmgm.ca',
}

// BetMGM sport IDs → competition IDs we care about
export const MGM_LEAGUES: { sportId: number; competitionId: number; leagueSlug: string; name: string }[] = [
...
export interface MGMGameMarket {
...
async function fetchFixtureIds(sportId: number, competitionId: number): Promise<{ id: string; name: string; participants: any[]; startDate: string }[]> {
  ...
  try {
    ...
    console.error(`BetMGM fixtures sportId=${sportId}:`, e)
    return []
  }
}
...
function parseFixtureMarkets(data: any, leagueSlug: string): MGMResult | null {
  ...
  const gameMarkets: MGMGameMarket[] = []
