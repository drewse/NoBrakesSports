...
function extractMarkets(item: any, eventId: string, leagueSlug: string): CanonicalMarket[] {
  ...
  if (ml) {
    const outcomes = open(ml)
      ...
      .map((o: any) => { const p = parseOdds(o.oddsAmerican); return isNaN(p) ? null : { side: getSide(o.type), label: o.label, price: p, impliedProb: americanToImplied(p) } })
      .filter(Boolean) as CanonicalOutcome[]
    if (outcomes.length >= 2) out.push({ eventId, marketType: 'moneyline', shape: detectMarketShape(leagueSlug, 'moneyline'), outcomes, lineValue: null, sourceSlug: 'betrivers_on', capturedAt: now })
  }
  ...
  if (sp) {
    ...
    for (const o of open(sp).filter((o: any) => ['OT_ONE','OT_TWO'].includes(o.type))) {
      const p = parseOdds(o.oddsAmerican); if (isNaN(p)) continue
    ...
    if (outcomes.length >= 2 && lineValue !== null) out.push({ eventId, marketType: 'spread', shape: '2way', outcomes, lineValue, sourceSlug: 'betrivers_on', capturedAt: now })
  }
  ...
  if (tot) {
    ...
    if (outcomes.length >= 2 && lineValue !== null) out.push({ eventId, marketType: 'total', shape: '2way', outcomes, lineValue, sourceSlug: 'betrivers_on', capturedAt: now })
  }
  ...
  async fetchEvents(): Promise<FetchEventsResult> {
    ...
    for (const res of results) {
      ...
      for (const item of data.events ?? []) {
        ...
        allEvents.push(normalizeEvent({ externalId: String(ev.id), homeTeam: ev.homeName, awayTeam: ev.awayName, startTime: ev.start, leagueSlug: target.slug, sourceSlug: 'betrivers_on' }))
        allMarkets.push(...extractMarkets(item, String(ev.id), target.slug))
        count++
      }
      console.log(`[betrivers] ${target.slug}: ${count} events`)
    }

    console.log(`[betrivers] done: ${allEvents.length} events, ${allMarkets.length} markets, ${errors.length} errors in ${Date.now() - start}ms`)
