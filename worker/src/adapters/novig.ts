...
// Team abbreviation → full name per league. Novig's GraphQL sometimes
// returns only `symbol`/`abbreviation` for an event's teams (no long_name),
// which leaks "ATL vs NYK" style titles into our events table and creates
// duplicate event rows alongside the canonical "Atlanta Hawks vs New York
// Knicks". Hydrate via these maps before emitting; if an abbr is missing
// we skip the event rather than write a stub with a short title.
const NBA_ABBR_TO_FULL: Record<string, string> = {
...
const MLB_ABBR_TO_FULL: Record<string, string> = {
...
const NHL_ABBR_TO_FULL: Record<string, string> = {
...
const NFL_ABBR_TO_FULL: Record<string, string> = {
...
const ABBR_MAPS: Record<string, Record<string, string>> = {
    ...
    if (typeof id === 'string' && typeof start === 'string' && game && league && !out.has(id)) {
      ...
      if (homeName && awayName) {
        out.set(id, {
          ...
          homeTeamSymbol: homeSym,
          awayTeamSymbol: awaySym,
        })
      }
    }
