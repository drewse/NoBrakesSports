...
const CA_BOOK_TRACKER: CaBookEntry[] = [
  ...
  { name: 'BetMGM ON',     slug: 'betmgm_on',    platform: 'Entain CDS',    difficulty: 'medium', status: 'live',        deployedTo: 'railway', gameLevel: true,  props: false, frequency: '3 min',  notes: '~30 events/tick. NBA fixture-list flakes 1 error/tick.' },
  ...
  { name: '888sport',      slug: '888sport',     platform: 'Spectate',      difficulty: 'medium', status: 'partial',     deployedTo: 'railway', gameLevel: true,  props: false, frequency: '5 min',  notes: '14 events + 38 game markets via spectate-web. Intermittent ERR_CONNECTION_CLOSED (~1 in 3 cycles).' },
  { name: 'TonyBet',       slug: 'tonybet',      platform: 'BetConstruct',  difficulty: 'hard',   status: 'partial',     deployedTo: 'railway', gameLevel: false, props: false, frequency: '5 min',  notes: '26 events flowing (tonybet.ca). Landing /api/event/list has no odds; need deeper nav to trigger odds relation.' },

  // ── Partial (Kambi adapters wired, awaiting live games) ───────────────
  { name: 'Proline (OLG)', slug: 'proline',      platform: 'Kambi',         difficulty: 'easy',   status: 'partial',     deployedTo: 'railway', gameLevel: true,  props: false, frequency: '3 min',  notes: 'Kambi olgsportscaon. 1 NBA event returned (no lines yet — off-season). NBA/MLB/NHL only — soccer leagues 404.' },
  { name: 'Bally Bet',     slug: 'ballybet',     platform: 'Kambi',         difficulty: 'easy',   status: 'partial',     deployedTo: 'railway', gameLevel: true,  props: false, frequency: '3 min',  notes: 'Kambi bcscaon (eu1 host, en_US). 1 NBA event returned (no lines yet). NBA/MLB/NHL only.' },

  // ── Infra-blocked (needs CA residential proxy or CA VPS) ──────────────
  { name: 'Caesars',       slug: 'caesars',      platform: 'Liberty',       difficulty: 'hard',   status: 'blocked',     deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'Initially cleared AWS WAF via IPRoyal residential (extracted 287 UUIDs). IPRoyal Starlink pool now collectively rejected at edge (ERR_TUNNEL_CONNECTION_FAILED on 3/3 retries). Same reputation issue as BetVictor. Parked CAESARS_ENABLED=0. Needs mobile/4G or different residential provider.' },
  { name: 'BetVictor',     slug: 'betvictor',    platform: 'Proprietary',   difficulty: 'hard',   status: 'blocked',     deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'IPRoyal Starlink residential pool collectively rejected at betvictor.com edge (ERR_TUNNEL_CONNECTION_FAILED on every session ID). Parser is DONE (HTML scrape + markets API confirmed) but needs mobile/4G proxy or different residential provider to reach the origin. Parked BETVICTOR_ENABLED=0.' },
  { name: 'bet365',        slug: 'bet365',       platform: 'Proprietary',   difficulty: 'hard',   status: 'blocked',     deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'Cloudflare 403 direct + via PacketStream. Needs residential CA proxy.' },
  { name: 'BET99',         slug: 'bet99',        platform: 'Cloudflare-gated', difficulty: 'hard', status: 'blocked',    deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'Hits CF challenge page on every cycle. Discovery captures only CF assets.' },
  { name: 'theScore Bet',  slug: 'thescore',     platform: 'Penn (Apollo)', difficulty: 'hard',   status: 'blocked',     deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'Initially cleared CF via IPRoyal residential (CompetitionPageSectionLinesTabNode fired, captured SGP section). IPRoyal Starlink pool now tunnel-rejected at edge. Parser is DONE (GraphQL persisted query, active-fetch + JWT replay verified) — blocked by pool reputation. Parked THESCORE_ENABLED=0.' },

  // ── Hard to implement (no clean API — needs HTML scrape) ──────────────
  { name: 'Betano',        slug: 'betano',       platform: 'Kaizen (SSR)',  difficulty: 'hard',   status: 'in_progress', deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'Initial events SSR-rendered into HTML; no events-list XHR exists. Would need HTML parsing. BetBuilder XHR (/api/betbuilderplus/event) only fires per-event on hover.' },
...
const USA_BOOK_TRACKER: UsaBookEntry[] = [
  ...
  { name: 'Kalshi',              slug: 'kalshi',             operator: 'KalshiEX (CFTC DCM)',             url: 'https://kalshi.com',                 platform: 'Event contract exchange',    category: 'exchange', states: ['ALL'], status: 'planned', notes: 'CFTC-regulated, available in all 50. Sports contracts as of 2025.' },
  { name: 'Polymarket',          slug: 'polymarket-us',      operator: 'Polymarket (QCX)',                url: 'https://polymarket.com',             platform: 'Prediction market',          category: 'exchange', states: ['ALL'], status: 'planned', notes: 'CFTC-regulated US access via QCX acquisition.' },
  { name: 'Robinhood Predict',   slug: 'robinhood-prediction', operator: 'Robinhood Derivatives',         url: 'https://robinhood.com/prediction-markets', platform: 'Kalshi-powered',       category: 'exchange', states: ['ALL'], status: 'planned', notes: 'White-labeled Kalshi.' },
