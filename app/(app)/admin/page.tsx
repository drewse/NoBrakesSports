...
const CA_BOOK_TRACKER: CaBookEntry[] = [
  ...
  { name: 'Miseojeu',      slug: 'miseojeu',     platform: 'Loto-Québec',   difficulty: 'hard',   status: 'blocked',     deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'Railway discovery adapter wired (worker/adapters/miseojeu.ts) with PacketStream CA. Returns ERR_EMPTY_RESPONSE at TCP — Loto-Québec Azure gateway drops the PacketStream CIDR. Needs IPRoyal CA mobile.' },
  { name: 'PowerPlay',     slug: 'powerplay',    platform: 'Unknown (SBTech/Kambi?)', difficulty: 'hard', status: 'blocked', deployedTo: 'railway', gameLevel: false, props: false, frequency: null,     notes: 'Railway discovery adapter wired. www.powerplay.ca returns ERR_EMPTY_RESPONSE via PacketStream CA — same TCP-drop behaviour as Miseojeu. Needs IPRoyal CA mobile.' },
...
const USA_BOOK_TRACKER: UsaBookEntry[] = [
  ...
  { name: 'Novig',               slug: 'novig',              operator: 'Novig Inc.',                      url: 'https://app.novig.us',               platform: 'Hasura GraphQL + REST',      category: 'exchange', states: ['CO','NJ'], status: 'live', notes: 'Railway Playwright adapter, direct IP, 5-min cadence. Writes ~60 game markets + ~25 player props per cycle across MLB/NBA; moneyline, spread, total, MONEY_1H, SPREAD_1H, TOTAL_1H, TEAM_TOTAL, FIRST_INNING_TOTAL, CHAMPIONSHIP_WINNER (futures) + 20+ player-prop types. Active-batch replay of /nbx/v1/markets/book/batch with 404 peel-off for stale order books (~40 bad-id recoveries per cycle).' },
  { name: 'Prophet Exchange',    slug: 'prophet-exchange',   operator: 'Prophet Sports Exchange',         url: 'https://www.prophetx.co',            platform: 'Nuxt SPA + Pusher WS',       category: 'exchange', states: ['IN','NJ','OH'], status: 'blocked', notes: 'Deep-probed — blocked on auth flow. App at prophetx.co uses Pusher (cluster mt1, key c975574818f436e8dd4a) but anonymous sessions only subscribe to the private #server-to-user-{uuid} channel with no data frames. REST /trade/public/api/v1/events fires in some sessions but is SPA-interaction-gated (captured once, not reproducibly). Unblock paths: (1) real auth session, (2) automated page.click into event cards, (3) reverse-engineer public Pusher channel names. Worker runs at 30-min cadence with WS frame logging — parked until we pick one unblock path.' },
  ...
  { name: 'BetParx',             slug: 'betparx',            operator: 'Parx Casino',                     url: 'https://play.betparx.com',           platform: 'Kambi + Cloudflare',         category: 'regional', states: ['IL','MD','NJ','OH','PA'],                  status: 'blocked', notes: 'Two paths exhausted: (1) Kambi /parx/ listView 429s on every IP class (endpoint-level rate limit), (2) play.betparx.com Railway Playwright session returned ERR_EMPTY_RESPONSE on direct IP AND PacketStream US — Cloudflare TCP-drops the CIDR. Needs IPRoyal US mobile (one env var away from live — same upgrade unlocks BetOnline/LowVig/Sportsbetting.ag/Hard Rock/MyBookie/Bookmaker/BetUS).' },
