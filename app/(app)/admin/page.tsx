...
const USA_BOOK_TRACKER: UsaBookEntry[] = [
  ...
  { name: 'PrizePicks',          slug: 'prizepicks',         operator: 'PrizePicks LLC',                  url: 'https://www.prizepicks.com',         platform: 'DFS pick-em',                category: 'dfs-sweepstakes', states: ['AK','CA','DC','FL','GA','IA','IL','IN','KS','KY','MN','NC','ND','NE','NM','OK','OR','RI','SC','SD','TX','UT','VA','WI'], status: 'in_progress', notes: 'Vercel cron adapter shipped (lib/pipelines/adapters/prizepicks.ts). Fetches api.prizepicks.com/projections per league (NBA/MLB/NHL/NFL), resolves player+game+team via JSON:API included[], filters to odds_type=standard + status=pre_game, maps 35 stat types → prop_category. Model A: line_value populated, over/under prices NULL (pick-em has no per-leg odds). Awaiting first sync-props fire to confirm event matches.' },
