...
const USA_BOOK_TRACKER: UsaBookEntry[] = [
  ...
  { name: 'Sleeper Picks',       slug: 'sleeper',            operator: 'Sleeper Fantasy',                 url: 'https://sleeper.com/picks',          platform: 'DFS pick-em',                category: 'dfs-sweepstakes', states: ['AK','CA','FL','GA','IL','KS','KY','MN','NC','ND','NE','NM','OK','OR','RI','SC','TX','UT','WI'], status: 'in_progress', notes: 'Standalone cron /api/cron/sync-sleeper, 5-min schedule offset from Underdog. Adapter pulls /lines/available (~5MB) + per-sport /players/{sport} indexes for name resolution. Model B — converts payout_multiplier decimal to American odds. Sleeper does not expose home/away or start_time, so cron matches by (league, sorted team-abbr pair) against upcoming events instead of the canonical-key tuple.' },
