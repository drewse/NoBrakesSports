...
const USA_BOOK_TRACKER: UsaBookEntry[] = [
  ...
  { name: 'LowVig',              slug: 'lowvig',             operator: 'BetOnline group',                 url: 'https://www.lowvig.ag',              platform: 'Proprietary (BetOnline)',    category: 'offshore', states: ['ALL'], status: 'blocked', notes: 'BetOnline runs a Kambi white-label at api-offering-ext.betonline.ag. Direct curl returns HTTP 401 Unauthorized — API is token/cookie-protected (session-bound JWT). Needs DevTools capture from a browser session to reverse the auth flow before an adapter can ship. Same stack as LowVig/BUSR — one auth reverse unlocks all three.' },
]
